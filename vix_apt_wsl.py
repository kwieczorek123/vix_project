...
# Rename columns
data_pct.columns = ['VIX', 'Consumer Confidence', 'High Yield Spread', 'Job Openings', 'VVIX', 'Recession Probabilities']
...
returns = data_pct.dropna()

# Define the independent variables as the macroeconomic factors
X = sm.add_constant(
    data_pct[['VIX', 'Consumer Confidence', 'High Yield Spread', 'Job Openings', 'VVIX', 'Recession Probabilities']].dropna())

# Use weighted least squares to estimate the betas for all assets simultaneously
model = sm.WLS(returns, X, weights=1 / returns.std()).fit()

# Print the model summary
print(model.summary())

# Save the beta coefficients for each asset
betas = pd.DataFrame(model.params[1:], columns=['Beta'])
betas.index = returns.columns[1:]
...
data_pct_rf = pd.read_csv('data_pct_rf.csv', index_col=0, parse_dates=True)
# Define the dependent variable as the historical excess returns of each asset

excess_returns = data_pct_rf.iloc[:, :-1]
excess_returns = excess_returns.subtract(data_pct_rf.iloc[:, -1], axis=0)
excess_returns.columns = excess_returns.columns.str.replace(' ', '_')
excess_returns.dropna(inplace=True)
# Define the independent variables as the macroeconomic factors

independent_variables = data_pct_rf.iloc[:, -1:]
independent_variables.columns = independent_variables.columns.str.replace(' ', '_')
independent_variables.dropna(inplace=True)
# Define the weights for the weighted least squares regression

weights = pd.DataFrame(1 / excess_returns.var(axis=0), index=excess_returns.columns, columns=['Weight'])
# Fit a weighted least squares regression model with excess returns as the dependent variable and macroeconomic
# factors as the independent variables

model = sm.WLS(excess_returns, sm.add_constant(independent_variables), weights=weights).fit()
# Print the model summary

print(model.summary())
# Calculate the betas for each asset
...
# Run a multivariate regression to estimate the betas for all assets simultaneously
model = sm.WLS(data_pct_rf.drop(columns=['risk free rate']), sm.add_constant(independent_variables),
               weights=np.sqrt(dtb3_monthly.values.flatten())).fit()

# Print the model summary
print(model.summary())

# Calculate the expected returns for each asset using the estimated betas and historical risk premiums
expected_returns = pd.DataFrame(index=betas.index, columns=['Expected Return'])
for asset in betas.index:
    expected_return = dtb3_monthly.iloc[-1]
    for factor in betas.columns:
